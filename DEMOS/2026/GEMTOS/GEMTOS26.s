...
* // GEMTOS26.PRG                  // *
***************************************
* // Asm Intro Code Atari ST v0.48 // *
* // by Zorro 2/NoExtra (31/10/25) // *
***************************************
...
CLEAN_TIMERS     equ 0           ; Clean Timers for...               *
**********************************************************************
TIMERC           equ 1           ; USe SNDH at 200 Hz with Timer C   *
R_FREQ           equ 200         ; SNDH replay frequency (MUSIC.MON) *
**********************************************************************
*              Notes : 0 = I use it / 1 = no need !                  *
**********************************************************************
SYNCHRO_WAIT      equ $900       ; Synchronysed elements adress      *
SCREEN_01         equ 0          ; Display intro screen              *
TEMPO_SCREEN_01   equ $055       ; Temporisation Screen #01          *
SCREEN_02         equ 0          ; Display noextra scrolling screen  *
TEMPO_SCREEN_02   equ $400       ; Temporisation Screen #02          *
SCREEN_03         equ 0          ; Display gemtos extra screen       *
TEMPO_SCREEN_03   equ $700       ; Temporisation Screen #03          *
SCREEN_04         equ 0          ; Display end screen                *
TEMPO_SCREEN_04   equ $701       ; Temporaisation Screen #04         *
**********************************************************************
...
**************************** MAIN LOOP ************************>
 ...
 IFEQ SCREEN_01
	...
	jsr	timer_Screen_01_start

	move.l	#TEMPO_SCREEN_01,SYNCHRO_WAIT
...
ecran_loop_01:
 ...
 IFEQ	SEEMYVBL
	move.l SCREEN_01_palette,$ffff8240.w ; init line of CPU
 ENDC
...
.next_key1:                        ;
 ...
 IFEQ SCREEN_02
	...
	lea	SCREEN_02_palette,a0         ; Put Default palette
	lea	$ffff8240.w,a1               ;
	movem.l	(a0),d0-d7               ;
	movem.l	d0-d7,(a1)               ;

	jsr	timer_Screen_02_start

	move.l	#TEMPO_SCREEN_02,SYNCHRO_WAIT
...
ecran_loop_02:
 ...
 IFEQ	SEEMYVBL
	move.l SCREEN_02_palette,$ffff8240.w ; init line of CPU
 ENDC
...
.next_key2:                        ;
 ...
 IFEQ SCREEN_03
	...
	lea	SCREEN_03_palette,a0         ; Put Default palette
	lea	$ffff8240.w,a1               ;
	movem.l	(a0),d0-d7               ;
	movem.l	d0-d7,(a1)               ;

	jsr	timer_Screen_03_start

	move.l	#TEMPO_SCREEN_03,SYNCHRO_WAIT
...
ecran_loop_03:
 ...
 IFEQ	SEEMYVBL
	move.l SCREEN_03_palette,$ffff8240.w ; init line of CPU
 ENDC
...
.next_key3:                        ;
 ...
 IFEQ SCREEN_04
	...
	lea	SCREEN_04_palette,a0         ; Put Default palette
	lea	$ffff8240.w,a1               ;
	movem.l	(a0),d0-d7               ;
	movem.l	d0-d7,(a1)               ;

	jsr	timer_Screen_04_start

	move.l	#TEMPO_SCREEN_04,SYNCHRO_WAIT
...
ecran_loop_04:
 ...
 IFEQ	SEEMYVBL
	move.l pal_haut,$ffff8240.w ; init line of CPU
 ENDC

* < Put your code here >

	jsr	display_dotWaves
...
.loop:	tst.b	(a0)                 ;
 ...
 IFEQ TIMERC
timer_c:
	sub.w   #R_FREQ,tccount          ; you can use any replay rate here less than or equal to 200Hz
	bgt.s   .nocall                  ; dont call zak this time
	add.w   #R_FREQ,tccount          ; syntheses any freq from 200hz system tick
...
.nocall:
	move.l  oldtc,-(sp)              ; go to old vector (system friendly ;) )
	rts
...
***************************************************************
	...
	include "E:\GEMTOS\_asm\CLEARFUL.ASM"
...
***************************************************************
 ...
 IFEQ SCREEN_01
	include "E:\GEMTOS\_asm\SCREEN01.ASM"
	include "E:\GEMTOS\_asm\TXTASCII.ASM"
 ENDC
 IFEQ SCREEN_02
	include "E:\GEMTOS\_asm\SCREEN02.ASM"
	include "E:\GEMTOS\_asm\SCROLV1P.ASM"
 ENDC
 IFEQ SCREEN_03
	include "E:\GEMTOS\_asm\SCREEN03.ASM"
	include "E:\GEMTOS\_asm\SQUARES.ASM"
	include "E:\GEMTOS\_asm\SCROL8x8.ASM"
	include "E:\GEMTOS\_asm\RASTERS.ASM"
 ENDC
 IFEQ SCREEN_04
	include "E:\GEMTOS\_asm\SCREEN04.ASM"
	include "E:\GEMTOS\_asm\DOTWAVES.ASM"
	include "E:\GEMTOS\_asm\PRINT.ASM"
 ENDC
...
* << Full data here >>
 ...
 IFEQ SCREEN_02
SCREEN_02_Logo:	even
	incbin	"E:\GEMTOS\_pic\50PC.IMG" ; 320x37
SCREEN_02_palette:
 include	"E:\GEMTOS\_pic\50PC.PAL"
 ENDC
 IFEQ SCREEN_03
SCREEN_03_Logo:	even
	incbin	"E:\GEMTOS\_pic\GEMTOS.IMG" ; 320x46
SCREEN_03_palette:
 include	"E:\GEMTOS\_pic\GEMTOS.PAL"
*
Pal_Squares:
	dc.w	$0012,$0123,$0234,$0345,$0456,$0544,$0544,$0544
 ENDC
 IFEQ SCREEN_04
 ENDC
...
MUSIC:
 IFEQ TIMERC
	incbin "E:\GEMTOS\_snd\4K3.THK"            ; SNDH Music played at the VBL
 ELSEIF
	incbin "E:\GEMTOS\_snd\4K2.THK"            ; SNDH Music played at the VBL
 ENDC
...
* << Full data here >>
 ...
 ENDC
 IFEQ SCREEN_02
buffer_Scroll:	ds.b	6400
buffer_Charac:	ds.b	100 
 ENDC
 IFEQ SCREEN_03
Buffer_FrameS:	ds.b	160*200*8
decal:					ds.w 	1
buffer_carac:		ds.w 	8
buffer_scroll:	ds.l	8*10
 ENDC
 IFEQ SCREEN_04
 ENDC
