...
play_Texte_Circle:
      ...
      MOVEQ     #0,D1 
      MOVEQ     #0,D2 
      MOVEA.L   PTR_ASCII(PC),A1
      MOVE.B    TEST_CARAC(PC),D0
      TST.B     D0
      BNE.S     .yes_text 
.reset_text:
      MOVE.B    #8,D0 
      MOVEA.L   PTR_TEXTE_CIRCLE(PC),A0
      MOVE.B    (A0)+,D1
      TST.B     (A0)
      BNE.S     .no_reset_text
      LEA       TEXTE_CIRCLE(PC),A0
.no_reset_text:
      MOVE.L    A0,PTR_TEXTE_CIRCLE
      LEA       ASCII(PC),A1
      LEA       CHARACTERS(PC),A2
      MOVE.B    0(A2,D1.W),D1 
      EXT.W     D1
      ASL.W     #3,D1 
      LEA       0(A1,D1.W),A1 
...
.yes_l8:LEA       1(A0),A0
      DBF       D7,loop_CIRCLE
.recopy_from_buffer:
      move.l	#buffer_Circle,a0                ; FROM ADRESS DATA
