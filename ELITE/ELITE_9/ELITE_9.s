...
Inits:
 ...
 ENDC
	...
	BSR	INIT_WAVE                    ; Inits des vagues
...
Vbl:
 ...
 IFEQ	NO_BORDER
	...
	move.b		#$04,$fffa21	; Pour pouvoir changer TBDR.
	move.l		#Hbl_Ascii,$120	; Nouvelle routine hbl.
...
wait:
 ...
 cmpi.b		#$50,d0		; Milieu de la ligne?
 blt		  wait
 subi.l		#$50,d0		; d0=nbre de nops a executer.
 jmp		  $0(a0,d0)
sync:
 dcb.w    50,$4E71                 ; 50 nops
...
one_line:
 ...
 move.w		(a0)+,(a1)	; +3=122.
 dcb.w    2,$4E71                 ; 2 NOP
 dbra     d0,one_line	; +4=128. 		
 move.w		#COLORF,(a1)	;	8240.w

 movem.l	(sp)+,d0/a0-a1

 bclr		#$0,$fffa0f	; Signale la fin de l'interupt.
...
Save_and_init_st:
 ...
 IFEQ	NO_BORDER
	sf	$fffffa21.w                  ; Timer B data (number of scanlines to next interrupt)
	sf	$fffffa1b.w                  ; Timer B control (event mode (HBL))
	lea	Hbl_Ascii(pc),a0             ; Launch HBL
	move.l	a0,$120.w                ;
	bset	#0,$fffffa07.w             ; Timer B vector
	bset	#0,$fffffa13.w             ; Timer B on
	bclr	#3,$fffffa17.w             ; Automatic End-Interrupt hbl ON
...
*********************************************************************
*                    VERTICAL WAVE ONE BITPLANE                     *
*                         ATOMUS/NOEXTRA                            *
*********************************************************************
DISPLAY_WAVE:
      CMPI.W    #30,count          ; play with rasters !
      BGE.S     .no_color
      ADD.W     #1,count
      MOVE.L    #entre,a0
      MOVE.W    count,d0
...
PLAY_WAVE:
      LEA       WAVE_DATA_1+8*4,A2 ; play vertical wave
      LEA       WAVE_DATA_2,A3
      ADDA.W    WAVE_BSS_1,A2
      ADDA.W    WAVE_BSS_2,A3
      MOVEA.L   physique(pc),A0
      LEA       160*6(A0),A0
      MOVE.W    #182-1,D1          ; nb line to display
.loop:LEA       WAVE_BSS_3,A1
      MOVE.W    (A2)+,D0
      ADD.W     (A3)+,D0
      ADDA.W    D0,A1
i set 0
...
i set i+8
 endr
      LEA       160(A0),A0
      DBRA      D1,.loop
      ADDI.W    #2,WAVE_BSS_1      ; speed
      SUBI.W    #2,WAVE_BSS_2      ; speed
      CMPI.W    #720*2-8*3,WAVE_BSS_1 
      BLT.S     .NO_INIT_WAVE 
      MOVE.W    #0,WAVE_BSS_1
      MOVE.W    #720,WAVE_BSS_2 
.NO_INIT_WAVE:
      RTS
...
Default_palette:
	dc.w	COLORF,$0C06,$0777,$0666,$0557,$0447,$0337,$0A93
	dc.w	$0A93,$0A93,$0A93,$0A93,$0A93,$0A93,$0A93,$0A93
...
MUSIC:
	incbin "ZIGGY.SND"            ; SNDH Music played at the VBL
	even
